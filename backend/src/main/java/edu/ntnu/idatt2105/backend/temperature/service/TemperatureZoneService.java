 ...
 * all linked {@link edu.ntnu.idatt2105.backend.task.model.TaskTemplate} records so that existing
 * checklist tasks stay in sync. Every mutating operation invalidates the checklist cache state for
 * the affected compliance area.
 */
@Service
@AllArgsConstructor
public class TemperatureZoneService {
   ...
   */
  public TemperatureZoneResponse createZone(CreateTemperatureZoneRequest request,
  ...
  /**
   * Updates an existing temperature zone and propagates the new target range to all linked task
   * templates.
   *
   ...
   */
  public TemperatureZoneResponse updateZone(Long zoneId, CreateTemperatureZoneRequest request,
      JwtAuthenticatedPrincipal principal) {
    UUID orgId = principal.requireOrganizationId();
  ...
  /**
   * Returns all temperature zones for the caller's organisation in the given module, ordered by
   * zone type then name.
   *
   ...
   */
  public List<TemperatureZoneResponse> getAllZones(IcModule module,
      JwtAuthenticatedPrincipal principal) {
    UUID orgId = principal.requireOrganizationId();
    return temperatureZoneRepository
        .findAllByOrganizationIdAndComplianceAreaOrderByZoneTypeAscNameAsc(orgId,
