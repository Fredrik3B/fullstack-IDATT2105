...
/**
 * Utility for converting between period strings, {@link ChecklistFrequency} values, and period
 * keys.
 *
...
public final class PeriodKeyUtil {
  ...
  /**
   * Converts a user-facing period string ({@code "daily"}, {@code "weekly"}, {@code "monthly"}) to
   * the corresponding {@link ChecklistFrequency}.
   *
  ...
  public static void validatePeriodKey(String periodKey, ChecklistFrequency frequency) {
    ...
    if (freq == ChecklistFrequency.MONTHLY) {
			if (!key.matches("\\d{4}-\\d{2}")) {
				throw new IllegalArgumentException("Invalid monthly periodKey: " + key);
			}
    ...
    if (freq == ChecklistFrequency.WEEKLY) {
			if (!key.matches("\\d{4}-W\\d{2}")) {
				throw new IllegalArgumentException("Invalid weekly periodKey: " + key);
			}
      ...
      return;
		...
		if (!key.matches("\\d{4}-\\d{2}-\\d{2}")) {
			throw new IllegalArgumentException("Invalid daily periodKey: " + key);
		}
  ...
  private static String normalizePeriod(String period) {
    String raw = (period == null ? "" : period).trim();
		if (raw.isEmpty()) {
			return "daily";
		}
    String lower = raw.toLowerCase(Locale.ROOT);
		...
		if (lower.equals("daily") || lower.equals("weekly") || lower.equals("monthly")) {
			return lower;
		}
