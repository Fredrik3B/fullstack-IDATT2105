 ...
 * ({@code Last-Modified} / {@code If-Modified-Since}). Every mutating checklist operation calls
 * {@link #touch} to advance the timestamp so clients know to re-fetch.
 *
 * <p>Timestamps are kept at second precision to comply with the HTTP date header format.
 * If two mutations arrive within the same second the timestamp is advanced by one second to
...
public class ChecklistCacheStateService {
  ...
  public Instant getLastModified(UUID organizationId, ComplianceArea complianceArea) {
    ...
    ComplianceArea safeComplianceArea = Objects.requireNonNull(complianceArea,
        "complianceArea is required.");
   ...
   */
  private ChecklistModuleState createState(UUID organizationId, ComplianceArea complianceArea,
   ...
   * the current second is used. Otherwise the previous second is incremented by one to guarantee
   * strict ordering.
   *
