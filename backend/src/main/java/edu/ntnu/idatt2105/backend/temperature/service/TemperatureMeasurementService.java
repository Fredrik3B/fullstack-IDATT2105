 ...
 * template). If the caller supplies an explicit {@code periodKey} it must match the task's own
 * period key; otherwise the task's period key is used automatically.
 */
@Service
@AllArgsConstructor
public class TemperatureMeasurementService {
   ...
   * and the task is a temperature-control task. If a {@code periodKey} is supplied it must match
   * the task's own period key.
   *
   * @param request   measurement data including module, checklist/task IDs, value, and optional
   *                  period key
   * @param principal the authenticated principal used to resolve the organisation and recorded-by
   *                  user
   * @return the persisted measurement as a response DTO with deviation flag
   * @throws IllegalArgumentException if validation fails (checklist/task not found, wrong module,
   *                                  inactive task, etc.)
   */
  public TemperatureMeasurementResponse createMeasurement(
    ...
    ChecklistModel checklist = checklistRepository.findByIdAndOrganization_Id(request.checklistId(),
            orgId)
    ...
    if (!task.isActive()) {
      throw new IllegalArgumentException(
          "Temperature can only be logged for active checklist tasks.");
    }
    if (!isTemperatureTask(task)) {
      throw new IllegalArgumentException(
          "Temperature can only be logged for temperature-controlled tasks.");
    }
    ...
    LOGGER.info("Measurement saved: id={} orgId={} taskId={}", saved.getId(), orgId,
        request.taskId());
    return temperatureMapper.toMeasurementResponse(saved, request.module());
  }

  /**
   * Returns temperature measurements for the caller's organisation filtered by module and date
   * range.
   *
   * <p>If {@code from} is {@code null} it defaults to {@link java.time.LocalDateTime#MIN};
   * if {@code to} is {@code null} it defaults to now. Results are ordered by {@code measuredAt}
   * descending.
   *
  ...
  /**
   * Returns {@code true} if the task's template has any temperature-control fields set (unit,
   * targetMin, or targetMax).
   *
